import type { Copy } from "../i18n";

export const ko: Copy = {
  languageName: "한국어",
  languageSwitchLabel: "언어 변경",
  appIntroLabel: "Jabiko 소개",
  appTitle: "Jabiko · JLPT 자습실",
  appTagline: "JLPT N5〜N1 문법, 한자, 어휘, 유형별 연습 — 한자리에서 모두 연습하세요.",
  themeLight: "라이트 모드",
  themeDark: "다크 모드",
  furiganaShow: "후리가나 표시",
  furiganaHide: "후리가나 숨김",
  flowLabel: "학습 흐름",
  navResources: "자료",
  navResourcesWithCurrent: (page) => `자료(현재: ${page})`,
  navMore: "더보기",
  navMoreTools: "설정 및 도구",
  navMoreWithCurrent: (page) => `더보기(현재: ${page})`,
  navPartnership: "제휴 홍보",
  partnersTitle: "제휴 홍보",
  partnersIntro: "현재 Jabiko와 제휴 중인 곳을 정리했습니다.",
  partnersContactTitle: "Jabiko와 제휴하고 싶으신가요?",
  partnersContactBody: "제휴 문의는 이 주소로 보내 주세요.",
  loading: "불러오는 중…",
  updateAvailable: "새 버전이 있습니다 — 탭하여 업데이트",
  routeErrorTitle: "페이지를 불러오지 못했습니다",
  routeErrorBody: "브라우저가 이전 앱 파일을 사용 중일 수 있습니다. 새로고침하여 최신 버전을 불러오세요.",
  routeErrorReload: "새로고침",
  routeErrorClearCache: "캐시를 지우고 다시 불러오기",
  routeErrorGoHome: "홈으로 돌아가기",
  breadcrumbLabel: "현재 위치",
  home: "홈",
  learn: "학습",
  rules: "규칙표",
  rulesEyebrow: "책상 위 참고서",
  rulesPanelTitle: "동사 활용 치트시트",
  rulesPanelIntro: "시험이나 연습 중에 활용형이 떠오르지 않으면 바로 이 페이지를 펼치세요. 동사 3그룹 분류, ます／て・た, 가능형 / 의지형 / 수동형 / 사역형, 형용사·명사 활용, 단계별 과거 의무형, 그리고 N5–N4 핵심 문형 치트시트를 다룹니다.",
  challenge: "연습",
  mockExam: "유형별 연습",
  about: "소개",
  aboutTitle: "Jabiko 소개",
  aboutTagline: "Master Japanese verbs, one step at a time.",
  aboutNameTitle: "이름의 유래",
  aboutNameBody:
    "Jabiko(ジャビ子)는 처음에 일본어 동사 활용을 몸에 밸 때까지 연습하려는 작은 도구로 시작했습니다. 이름을 풀어 보면: JA는 Japanese, bi는 동사 활용(bi-yō)에서, 그리고 ko는 일본어 이름에 자주 보이는 그 귀여운 「子」예요. 그렇게 ジャビ子가 태어났습니다 — 머리에 활용표 노트를 얹고 균형을 잡으며, ます형, て형, 가능형을 하나씩 함께 짚어 주는 작은 도우미 로봇이죠.",
  aboutStoryTitle: "시작부터 지금까지",
  aboutStoryBody:
    "처음엔 기초 활용 연습뿐이었습니다. 시간이 지나면서 문장 빈칸 채우기, 문형 판별, 완전한 JLPT 문제 은행, 유형별 연습, 한자 읽기, 약점 복습, 학습 대시보드까지 — 한 걸음씩, 점점 더 완성도 있게 자라났습니다.",
  aboutAuthorTitle: "제작자",
  aboutAuthorName: "花雪 (HanaYukii)",
  aboutAuthorBody:
    "최상위권 알고리즘 대회 선수 출신으로, 석사를 마치고 바로 Google에 입사해 3년간 일한 뒤 퇴사했고 지금은 AI 스타트업의 Tech Lead입니다. 일본어는 10년 전 N2를 따고 묵혀 둔 오래된 자산이었는데, 2024년 일본 아이돌을 다시 좋아하게 되면서 되살렸습니다. Jabiko는 처음엔 순전히 제가 쓰려고 만든 도구였습니다 — 동사 활용부터 N1 문법까지, 틀린 문제는 자동으로 복습에 들어가고, 난이도는 정답 상황에 맞춰 조정되며, 문제는 AI로 대량 생성한 뒤 직접 검수합니다. 이후 사용자가 조금씩 늘면서 정식 프로젝트로 유지하고 있고, 문제와 기능을 계속 추가하는 중입니다.",
  aboutAuthorLink: "제작자에 대해 더 보기 →",
  aboutAuthorContact: "작성자에게 연락",
  aboutAuthor2Title: "제작자",
  aboutAuthor2Name: "塔奇巧克力（TachikoChoko）",
  aboutAuthor2Body:
    "라이브 스트리밍 플랫폼「初樂（TrueLoveLive）」에서 백엔드 엔지니어로 근무한 후, 冰角工作室에서 백엔드 리드로 주로 백엔드 시스템 개발을 담당했습니다. 데이터베이스 성능 최적화, 쿼리 속도, 시스템 아키텍처 같은 수면 아래 엔지니어링 디테일을 연구하는 것을 늘 좋아했습니다. 이름 속「塔奇」는《공각기동대》의 타치코마에서 따왔습니다. 타치코마의 풍부한 지식과 실전 능력, 그리고 긴장된 상황에서도 소풍 가듯 신나고 호기심 넘치게 전력으로 임하는 모습을 좋아합니다.《공각기동대》외에도 대하드라마《료마전》, 영화《하야부사: 머나먼 귀환》, 그리고《관료들의 여름》을 무척 좋아합니다. 현재 도쿄의 일본어학교에서 일본어를 배우고 있습니다. 처음은 단지 자신의 일본어 동사 활용에 대한 어려움 때문에, 편리하게 연습하고 반복 확인하며 조금씩 쌓아갈 수 있는 작은 도구를 만들고 싶었습니다. Jabiko는 바로 그 필요에서 자라났습니다: 먼저 자신을 위해 혼란스러운 활용을 정리하고, 그 후 같은 방식으로 일본어를 배우는 사람들에게도 함께 사용할 수 있도록 공개했습니다.",
  homeHeroTitle: "오늘은 무엇을 연습할까요?",
  homeHeroIntro: "기초 활용부터 N1 감각까지. 문법, 한자, 어휘, 유형별 연습 — 모두 한자리에.",
  homeGuideLink: "사용 설명서",
  homeHeroKicker: "무료 · 회원가입 없이 · N5〜N1",
  homeHowText: "처음이신가요? ① 레벨 선택 → ② 오늘의 학습 → ③ 오답 자동 복습",
  homeHowDismiss: "확인",
  homeFooterWish: "한 걸음씩 천천히. 행운을 빌어요 — 멋지게 합격하시길.",
  feedbackWish: "기능 요청",
  feedbackBug: "문제 신고",
  feedbackTitle: "의견 보내기",
  focusLabel: "집중",
  focusStartLabel: "집중 시작",
  focusEndLabel: "집중 종료",
  focusConfigureTitle: "집중 설정",
  focusConfigureFocusMinutes: "집중 시간(분)",
  focusConfigureBreakMinutes: "휴식 시간(분)",
  focusConfigureSave: "시작",
  focusBreakTitle: "휴식",
  focusBreakRestPrompt: "잠시 쉬고 스트레칭이나 물 한 잔 한 뒤, 다음 라운드를 시작하세요.",
  focusSkipBreak: "휴식 건너뛰기",
  focusNextCycle: "다음 라운드 시작",
  focusEndMode: "집중 모드 종료",
  focusSummaryFocus: "이번 집중",
  focusSummaryQuestions: "답변",
  focusSummaryAccuracy: "정답률",
  focusSummaryToday: "오늘 누계",
  focusAdvertisement: "광고",
  focusActiveTitle: "집중 진행 중",
  focusRemainingLabel: "남은 시간",
  focusCycleLabel: (cycle) => `${cycle}번째 라운드`,
  feedbackKind: { wish: "기능 요청", bug: "문제 신고", other: "기타" },
  feedbackPlaceholder: "보고 싶은 기능이든, 마주친 문제든 — 무엇이든 편하게 알려 주세요 (익명)…",
  feedbackContact: "연락처 (선택 — 답변을 받고 싶다면 남겨 주세요)",
  feedbackWantsReply: "답장을 받고 싶어요",
  feedbackWantsReplyHint: "로그인 상태면 계정으로 답장드립니다. 아니면 아래에 연락처를 남겨 주세요.",
  feedbackAnon: "익명으로 전송되며 로그인이 필요 없습니다.",
  feedbackDiagNote: "문제를 재현할 수 있도록 기술 환경(페이지·기기·브라우저·화면 크기 등)을 함께 보냅니다. 입력하신 내용은 포함되지 않습니다.",
  feedbackSend: "보내기",
  feedbackSending: "전송 중…",
  feedbackThanks: "잘 받았어요 — 의견 감사합니다!",
  feedbackClose: "닫기",
  feedbackError: "전송에 실패했어요. 잠시 후 다시 시도하거나,",
  feedbackFallback: "대신 GitHub으로 신고하기",
  feedbackYourAnswer: "내 답",
  reportQuestionCta: "이 문제 신고하기",
  reportTitle: "이 문제의 오류 신고",
  reportReasonLabel: "무엇이 잘못됐나요?",
  reportReasons: {
    wrongAnswer: "정답 / 정답 선택지가 틀림",
    awkwardMeaning: "문장이 어색함",
    confusingExplanation: "해설이 헷갈림",
    typo: "오타 또는 표현 문제",
    other: "기타"
  },
  reportDetailPlaceholder: "자세한 내용 추가 (선택)…",
  reportSend: "신고 보내기",
  homeContentStats: (total, examItems, vocab, kanjiReadings, patternChecks, chapters) =>
    `전체 ${total.toLocaleString()}문제 · 검정 문제 ${examItems} · 어휘 ${vocab} · 한자 읽기 ${kanjiReadings} · 문형 ${patternChecks} · 학습 챕터 ${chapters}`,
  homeCardStageLearn: "학습",
  homeCardStageChallenge: "연습",
  homeCardStageVocab: "암기",
  homeCardStageMock: "유형",
  homeCardStageReview: "복습",
  homeBannerReviewMain: (count) => `복습할 문제가 ${count}개 있어요`,
  homeBannerReviewSub: "세션을 넘어 쌓인 오답들 — 맞혀야만 빠져나가요.",
  homeBannerContinueMain: (chapter) => `이어서 학습: ${chapter}`,
  homeBannerContinueSub: "지난번에 끝내지 못한 챕터예요.",
  homeDailyMain: "오늘의 연습 시작",
  homeDailySub: "먼저 틀린 문제부터 정리한 뒤, 문법·어순·한자 읽기를 섞어 한 바퀴 돌아요.",
  homeConjugationMain: "동사 활용",
  homeConjugationSub: "동사와 목표 활용형을 보고 기억해서 직접 입력하세요.",
  homeStatsLabel: "전체 진행도",
  homeStatsAttempts: "푼 문제",
  homeStatsAccuracy: "전체 정답률",
  homeStatsChapters: "완료한 챕터",
  homeProgressLabel: "학습 진행도",
  homeStatsStreak: "연속 일수",
  homeStatsDue: "복습 예정",
  homeStatsMastered: "마스터함",
  homeStatsPoints: "획득 포인트",
  homeLevelLabel: "레벨별 정답률",
  homeLevelAnswered: (count) => `${count}문제 풀이`,
  homeTrendTitle: "일일 연습량",
  homeTrendRange: (days) => `최근 ${days}일`,
  homeTrendPeak: (count) => `최대 ${count}문제/일`,
  homeTrendDay: (date, count) => `${date}: ${count}문제`,
  homeTypeWeaknessTitle: "약한 문제 유형",
  typeBandLabels: { weak: "보강 필요", mid: "보통", strong: "탄탄함" },
  questionTypeLabels: {
    grammar: "문법",
    vocab: "어휘",
    kanji: "한자 읽기",
    syn: "유의어",
    usage: "용법",
    context: "문맥",
    read: "독해",
    order: "어순",
    text: "지문",
    cloze: "문장 빈칸",
    pattern: "문형",
    basic: "기초 · 어휘"
  },
  homeCardLearnTitle: "학습",
  homeCardLearnSub: "챕터별 활용과 문형 풀이 — 규칙을 읽고 나서 연습하세요.",
  homeCardLearnMeta: (completed, total) => `${total}챕터 중 ${completed}챕터 완료`,
  homeCardChallengeTitle: "연습",
  homeCardChallengeSub: "세 그룹: 데일리 루프 · N1〜N4 시험 대비 · 집중 드릴.",
  homeCardChallengeMeta: "JLPT 레벨과 문제 유형 자유 선택",
  homeCardMockTitle: "JLPT 유형별 연습",
  homeCardMockSub: "JLPT N1〜N5 섹션을 공식 구조에 따라 파트별로 풀어 나가요.",
  homeCardMockMeta: "공식 섹션 구조를 따릅니다",
  homeCardReviewTitle: "약점 복습",
  homeCardReviewSubActive: (count) => `${count}문제가 당신이 맞히길 기다리고 있어요.`,
  homeCardReviewSubEmpty: "지금 복습할 오답이 없어요.",
  homeCardReviewMeta: "틀리면 바로 오답 모음에 들어가요 · 한 번 맞히면 바로 빠져요",
  homeCardBookmarksTitle: "즐겨찾기",
  homeCardBookmarksSubActive: (count) => `${count}개의 별표 문제를 언제든 다시 연습.`,
  homeCardBookmarksSubEmpty: "연습 중 별표를 눌러 저장하고, 여기서 다시 연습하세요.",
  homeCardBookmarksMeta: "문제 페이지에서 ☆ 로 저장",
  homeQuickLinksLabel: "자료 찾기",
  quickLinkGrammar: "문형 데이터베이스",
  quickLinkKanji: "한자 읽기",
  quickLinkRules: "활용 빠른 참조표",
  homeGridLabel: "연습할 영역 선택",
  mockExamLevelLabel: "레벨",
  mockSectionTitle: "섹션별 연습",
  mockSectionIntro: "JLPT 문제 유형을 골라 그 섹션만 집중 연습하세요. 답을 고른 직후 바로 해설을 볼 수 있고, 오답은 곧장 약점 복습으로 들어갑니다.",
  mockSectionCount: (count) => `${count}문제`,
  mockSectionEmpty: "준비 중",
  kanji: "한자",
  kanjiTitle: "한자 음독 찾기",
  kanjiIntro: "음독(같은 소리 계열)으로 한자를 찾아 읽기를 확인하고, 탁음에 헷갈리지 마세요. 한자를 누르면 예시 단어가 보입니다.",
  kanjiSearchPlaceholder: "한자, 읽기, 뜻 검색",
  kanjiLevelAll: "전체",
  kanjiLevelFilter: "JLPT 레벨",
  kanjiOnyomiLabel: "음독",
  kanjiKunyomiLabel: "훈독",
  kanjiCellOnPrefix: "음",
  kanjiCellKunPrefix: "훈",
  kanjiReadingType: "읽기 유형",
  kanjiReadingOn: "음독",
  kanjiReadingKun: "훈독",
  kanjiExamplesLabel: "예시 단어",
  kanjiNoExamples: "(아직 예시 단어가 없어요)",
  kanjiSearchEmpty: "일치하는 한자를 찾을 수 없어요.",
  kanjiArrowHint: "팁: 키보드 ← / → 로 카드를 연달아 넘길 수 있어요.",
  kanjiLoadMore: (count: number) => `더 보기 (${count}자 남음)`,
  kanaPageTitle: "오십음도",
  kanaPageIntro: "히라가나·가타카나 대조표. 셀을 누르면 발음이 재생됩니다.",
  kanaHiragana: "히라가나",
  kanaKatakana: "가타카나",
  kanaGroupSeion: "청음",
  kanaGroupDakuon: "탁음·반탁음",
  kanaGroupYouon: "요음",
  learningRegion: "학습",
  chapterIndexLabel: "학습 챕터",
  chapterPrev: "이전 장",
  chapterNext: "다음 장",
  chapterEyebrow: "코스 챕터",
  chapterHeading: "한 번에 한 챕터씩 잠금 해제",
  chapterIntro: "챕터를 골라 규칙, 예문, 자주 빠지는 함정을 읽은 뒤 연습 페이지에서 풀어 보세요.",
  chapterStatusReference: "참고",
  chapterStatusComplete: "완료",
  chapterViewLabel: (title) => `보기: ${title}`,
  chapterPrereqHint: (titles) => `먼저 보면 좋아요: ${titles}`,
  chapterExampleLabel: (title) => `${title} 예문`,
  chapterPitfallsTitle: "자주 빠지는 함정",
  studyBeforeRecall: "먼저 학습하고, 그다음 떠올리기",
  learnTitle: "먼저 익히고, 그다음 도전",
  learnIntro: "처음에는 순서대로 진행하세요: 품사를 구분하고, 함께 묶이는 활용을 그룹으로 묶은 뒤, 객관식으로 확인합니다. 규칙이 손에 잡히면 그때 입력 연습으로 넘어가세요.",
  roadmapLabel: "추천 학습 순서",
  learningSteps: [
    {
      label: "먼저 분류",
      title: "표 암기부터 시작하지 마세요",
      body: "먼저 물어보세요: 이건 동사인가, い형용사인가, な형용사인가, 명사인가? 동사라면 1그룹, 2그룹, 3그룹 중 무엇인지 정하세요."
    },
    {
      label: "계열 고르기",
      title: "관련된 활용은 함께 묶어 두기",
      body: "て형 / た형은 한 세트, ない, ないで, なくて, なかった는 또 다른 한 세트예요. 한 계열이 어떻게 이어지는지 보는 것이 단어를 하나씩 외우는 것보다 낫습니다."
    },
    {
      label: "오답에서 캐내기",
      title: "틀렸다면 규칙을 읽어요",
      body: "연습 문제를 틀리면 정답과 규칙이 나타납니다. \"왜 이렇게 바뀌는가\"를 확인한 뒤 다음으로 넘어가세요."
    }
  ],
  step: "Step",
  verbGroupTitle: "동사를 먼저 세 그룹으로 분류하기",
  verbGroupIntro: "지금은 て형과 た형은 잊으세요. 모든 동사 문제의 첫걸음은 그저 분류입니다 — 그걸 맞히면 \"마지막 가나를 바꿀지\" 아니면 \"る를 떼어낼지\"를 알게 돼요.",
  teTaTitle: "て형과 た형은 하나의 표예요",
  teTaIntro: "1그룹 동사에서 어려운 부분은 음변화입니다. \"て / た를 한 쌍으로\" 외우세요 — 두 가지 규칙을 따로 익히지 마세요. 行く는 예외입니다: 行って, 行った.",
  negativeTitle: "모든 부정형은 먼저 ない형으로 돌아가요",
  negativeIntro: "당신을 헷갈리게 하는 \"て형/た형의 부정\"은 사실 て형이나 た형에서 나온 게 아니에요. 먼저 ない형을 만들고 거기서부터 쌓아 올리세요.",
  adjectiveTitle: "형용사와 명사를 함께 외우지 마세요",
  adjectiveIntro: "い형용사는 い를 떼고, な형용사와 명사는 だ, ではない, だった를 쓰는 \"명사절\"에 더 가깝게 움직입니다.",
  obligationPastTitle: "과거 의무는 마지막 부분을 보세요",
  obligationPastIntro: "「ならなければ」는 과거가 아니에요. 과거로 만들려면 앞의 조건형은 그대로 두고 마지막 「ならない」를 「ならなかった」로 바꾸세요.",
  tableEnding: "어미",
  tableTe: "て형",
  tableTa: "た형",
  tableExample: "예",
  teTaTableLabel: "1그룹 동사 て/た 음변화",
  drillGodanTeTa: "1그룹 て/た 연습",
  drillNegative: "부정형 연습",
  drillIAdjective: "い형용사 연습",
  drillNaAdjective: "な형용사 연습",
  drillAdverbial: "く/に 수식 연습",
  drillObligationPast: "과거 의무 연습",
  drillMasu: "ます형 연습",
  drillPlain: "보통형 연습",
  drillPotential: "가능형 연습",
  drillVolitional: "의지형 연습",
  drillConditional: "가정형(ば형) 연습",
  drillPassive: "수동형 연습",
  drillCausative: "사역형 연습",
  drillDesiderative: "たい・たがる 연습",
  drillPatternStarterDesu: "기본 문장 〜です 연습",
  drillPatternStarterParticles: "조사 は・を・に・が 연습",
  drillPatternN5Sonzai: "あります・います 연습",
  drillPatternN5Ichi: "위치어와 この・その 연습",
  drillPatternN5Joshi2: "조사 へ・で・と・や 연습",
  drillPatternN5Joshi3: "조사 の・も・か・から 연습",
  drillPatternN5Hikaku: "비교 문형 연습",
  drillPatternN5SukiDekiru: "호불호와 능력 연습",
  drillPatternN5Sasoi: "권유와 제안 연습",
  drillPatternN5Onegai: "요청과 조언 연습",
  drillPatternN5Riyuu: "이유와 역접 연습",
  drillPatternN5Toki: "시간과 경험 연습",
  drillPatternN5Keiyoushi: "형용사 연결과 변화 연습",
  drillPatternN5Josuushi: "조수사 연습",
  drillPatternN5Teido: "정도와 빈도 연습",
  drillPatternN4Ndesu: "설명의 んです 연습",
  drillPatternN4Suiryou: "추량과 원인 연습",
  drillPatternN4Ishi: "의지와 결정 연습",
  drillPatternN4Meirei: "명령과 금지 연습",
  drillPatternN4Shushoku: "명사 수식절 연습",
  drillPatternN4Kansetsu: "간접 의문 연습",
  drillPatternN4Fukugou: "복합동사 연습",
  drillPatternN4Henka: "변화 문형 연습",
  drillPatternN4Jikan: "시간 문형 연습",
  drillPatternN4Juju: "수수와 의뢰 연습",
  drillPatternN4Chikaku: "지각과 한정 연습",
  drillPatternTeKudasai: "문형 연습: 요청 / 허가 / 금지",
  drillPatternNakuteMoII: "문형 연습: ~하지 않아도 됨 vs ~해야 함",
  drillPatternTeMorau: "문형 연습: 주고받기 관점",
  drillPatternToOmou: "문형 연습: 인용 / 의견",
  drillPatternMaeAto: "문형 연습: 전후 순서",
  drillPatternNagaraTari: "문형 연습: 병렬 · 동시 동작",
  drillPatternTeAux: "문형 연습: 보조동사",
  drillN3Grammar: "N3 문법 연습하러 가기",
  drillKana: "오십음 읽기 연습",
  drillStarterVocab: "기초 단어 연습",
  startChallenge: "연습 시작",
  settingsLabel: "연습 설정",
  todayPractice: "오늘의 연습",
  practiceType: "연습 유형",
  practiceMode: "연습 모드",
  levelRange: "문제 풀(pool)",
  levelRangeOptions: { all: "전체", n1n2: "N1＋N2", n2n3: "N2＋N3", n3n4: "N3＋N4", n4n5: "N4＋N5", starter: "완전 초보" },
  levelOnboarding: {
    title: "레벨을 선택하세요",
    subtitle: "오늘의 연습과 각 문제 풀의 기본 난이도를 정합니다 — 언제든 바꿀 수 있어요.",
    beginner: "초급",
    beginnerHint: "N4・N5",
    lowerIntermediate: "초중급",
    lowerIntermediateHint: "N3・N4",
    intermediate: "중급",
    intermediateHint: "N2・N3",
    advanced: "고급",
    advancedHint: "N1・N2",
    starter: "완전 초보",
    starterHint: "제로부터",
    manageTitle: "목표 레벨",
    change: "변경",
    notSet: "미설정",
    chooseFirst: "먼저 레벨을 고르면 바로 오늘의 연습이 시작돼요."
  },
  sessionLength: "세트당 문제 수",
  sessionLengthAll: "전체",
  sessionLengthCustom: "직접 설정",
  sessionLengthCustomPlaceholder: "문제 수",
  ttsRate: "읽기 속도",
  ttsRatePresets: {
    normal: "표준",
    slow: "느리게",
    slower: "더 느리게"
  },
  ttsRateCustom: "사용자 지정",
  practiceFocus: "연습 초점",
  answerMode: "답변 방식",
  answerModes: { choice: "선택지", recall: "직접 입력" },
  verbGroup: "동사 그룹",
  targetForm: "목표 활용형",
  answered: "푼 문제",
  correctShort: "정답",
  reviewShort: "복습",
  accuracyLabel: "현재 정답률",
  accuracyShort: "정답률",
  scoreReportLabel: "오늘의 리포트",
  shareTitle: "결과 공유하기",
  shareText: (attempts, accuracy) =>
    `Jabiko에서 JLPT 준비 중이에요 — 오늘 ${attempts}문제를 정답률 ${accuracy}%로 풀었어요! 같이 일본어 연습해요👇`,
  shareFbHint: "FB를 누르면 게시글 텍스트가 자동으로 복사됩니다 → 페이스북 새 게시물 창을 열고 → 붙여넣기",
  shareOther: "다른 플랫폼에 공유",
  shareCopied: "게시글 텍스트가 복사됐어요!",
  shareSiteTitle: "Jabiko 공유하기",
  shareSiteText:
    "저는 Jabiko로 JLPT를 준비하고 있어요: N5〜N1 문법, 한자, 어휘, 유형별 연습를 한자리에서 — 같이 공부한다면 당신에게도 추천해요👇",
  donate: "Jabiko 후원하기",
  resetSession: "이번 세션 초기화",
  currentQuestion: "현재 문제",
  questionNumber: (value) => `${value}번 문제`,
  questionProgress: (value, total) => `${value} / ${total}번 문제`,
  answerOptions: "답 선택지",
  recallAnswerLabel: "활용한 형태 입력",
  recallAnswerPlaceholder: "답을 입력하세요",
  recallSubmit: "답 제출",
  recallKeyboardHint: "Enter로 답하고, 해설 뒤에 Enter를 다시 눌러 다음 문제로 이동",
  revealAnswer: "정답 보기",
  nextQuestion: "다음 문제",
  emptyState: "현재 설정에 맞는 문제가 없어요.",
  mistakesLabel: "오답",
  mistakeReview: "오답 복습",
  noMistakes: "이번 세션에는 아직 오답이 없어요.",
  correct: "정답",
  incorrect: "다시 생각해 봐요",
  revealed: "이 문제 기억해 두기",
  answerKey: "정답",
  feedbackOtherOptions: "다른 선택지",
  feedbackNoWord: "해당 단어 없음",
  vocabNotesLabel: "핵심 어휘",
  showHint: "힌트",
  hideHint: "힌트 숨김",
  grammarNoteCta: "문법 노트 보기",
  grammarNoteHide: "노트 숨김",
  grammarNoteFormation: "접속",
  grammarNoteUsage: "용법",
  grammarNoteExamples: "예문",
  grammarNoteConfusions: "헷갈리기 쉬운 점",
  grammarStudyLink: "이 문법 자세히 배우기",
  grammarIndexTitle: "JLPT 문형 데이터베이스",
  grammarIndexIntro: "N5〜N1 자주 출제되는 문형을 중국어 해설, 접속 규칙, 예문 및 드라마/애니메이션 대사와 함께 확인하세요.",
  grammarSearchPlaceholder: "문형, 의미, 태그 검색…",
  grammarSearchResults: "검색 결과",
  grammarSearchEmpty: "일치하는 문형이 없습니다.",
  grammarNoPatterns: "이 레벨에는 아직 문형 데이터가 없습니다.",
  grammarFilterMediaOnly: "영상 예문 있음",
  grammarFilterAllImportance: "모든 중요도",
  grammarFilterImportance: "중요도 필터",
  grammarMediaExamples: "드라마/애니메이션 대사",
  grammarDatabaseExamples: "예문",
  grammarRelatedPatterns: "유사 문형 비교",
  grammarCommonMistakes: "자주 하는 실수",
  grammarBrowseLevel: "보기",
  grammarPagerLabel: "문형 앞뒤 이동",
  grammarPrev: "이전",
  grammarNext: "다음",
  grammar: "문형",
  grammarImportanceMustKnow: "필수",
  grammarImportanceHighFreq: "고빈도",
  grammarImportanceUnderstand: "이해 수준",
  grammarImportanceReference: "참고",
  grammarMatchPattern: "문형",
  grammarMatchZh: "중국어 의미",
  grammarMatchJa: "일본어 의미",
  grammarMatchTag: "태그",
  grammarMatchId: "ID",
  grammarHasMedia: "미디어 예문 포함",
  grammarMatchFieldLabel: (field: string) => `일치: ${field}`,
  grammarConfidenceVerified: "확인됨",
  grammarConfidenceSubtitleVerified: "자막 확인",
  grammarConfidenceApproximate: "상황 유사",
  grammarConfidenceInspiredBy: "참고 개작",
  grammarSourceAnime: "애니메이션",
  grammarSourceDrama: "드라마",
  grammarSourceMovie: "영화",
  grammarSourceOther: "기타",
  keyboardHint: "데스크톱 단축키: 1–4로 답하고, Enter로 다음 문제",
  focusSummaryEmpty: "이 초점에 사용할 수 있는 활용형이 없어요",
  modeOptions: {
    daily: { title: "오늘의 연습", subtitle: "복습 우선 · 문법 / 어순 / 한자 읽기 혼합 한 바퀴" },
    kana: { title: "오십음", subtitle: "히라가나/가타카나 읽기 인식" },
    starter: { title: "기초 단어", subtitle: "입문 95단어 · 뜻 고르기" },
    basic: { title: "기초 활용", subtitle: "품사별 활용 연습 · 교과서 어휘" },
    cloze: { title: "문장 빈칸", subtitle: "N5 문형 · 〜てください / 〜たいです" },
    pattern: { title: "문형 연습", subtitle: "N5/N4 문형 판별 · 관점 / 허가 / 인용 / ~하지 않아도 됨" },
    exam: { title: "전체 문제 은행", subtitle: "주로 N1〜N3 · 문법 / 어순 / 지문 / 어휘 / 한자 읽기" },
    examN1: { title: "N1 대비", subtitle: "N1＋N2 혼합 · 문법 / 어순 / 지문 / 어휘 / 한자 읽기" },
    examN2: { title: "N2 대비", subtitle: "N2＋N3 혼합 · 문법 / 어순 / 지문 / 어휘 / 한자 읽기" },
    examN3: { title: "N3 대비", subtitle: "N3＋N4 혼합 · 문법 / 지문 / 어휘 / 한자 읽기" },
    examN4: { title: "N4 대비", subtitle: "N4＋N5 혼합 · 조사 / 기초 문법 / 어휘 / 한자 읽기" },
    vocab: { title: "어휘 읽기", subtitle: "N1–N5 한자어 · 올바른 읽기(よみ) 고르기" },
    review: { title: "약점 복습", subtitle: "틀린 문제를 맞힐 때까지 다시 연습" },
    bookmarks: { title: "즐겨찾기", subtitle: "별표 표시한 문제를 다시 연습" }
  },
  modeGroups: { dailyGroup: "매일", examGroup: "시험 대비", focusedGroup: "분야별 연습" },
  modeQuestionCount: (count) => `풀에 ${count}문제`,
  homeCardVocabTitle: "어휘 읽기",
  homeCardVocabSub: "N1–N5 한자어의 빠른 읽기 확인. 뜻은 전체 은행에서 문장으로 출제되며, 여기서는 순수하게 よみ만 다룹니다.",
  homeCardVocabMeta: "통근길 자투리 시간에 딱 좋아요",
  homeCardVocabTitleStarter: "기초 단어",
  homeCardVocabSubStarter: "입문 95단어: 단어를 보고 뜻 고르기. 전부 가나, 한자 몰라도 OK.",
  homeCardVocabMetaStarter: "오십음을 막 뗀 분께",
  homeBannerNextLearnMain: "입문 코스 시작하기",
  homeBannerNextLearnSub: "오십음 → 기초 단어 → 기본 문장, 한 걸음씩.",
  homeBannerNextExamMain: (band) => `${band} 대비 시작하기`,
  homeBannerNextExamAllMain: "종합 문제 은행 시작하기",
  homeBannerNextExamSub: "목표 레벨에 맞춰 종합 문제를 연습하세요.",
  dashboardEyebrow: "이어서 학습",
  dashboardReviewPending: (count) => `아직 복습할 문제가 ${count}개 남았어요`,
  dashboardReviewEmpty: "복습할 오답이 남지 않았어요 🎉",
  dashboardReviewCta: "지금 복습",
  dashboardNextChapterLabel: "다음 챕터 추천",
  dashboardStatsAttempts: (count) => `${count}문제 완료`,
  dashboardStatsAccuracy: (percent) => `전체 정답률 ${percent}%`,
  reviewEmptyState: "지금은 틀린 문제가 없어요. 틀린 문제는 바로 여기에 나타나고, 한 번 맞히면 빠져요.",
  reviewEmptyCta: "문제 은행으로 가기",
  bookmarkAdd: "즐겨찾기",
  bookmarkRemove: "저장됨",
  bookmarksEmptyState: "아직 저장한 문제가 없어요. 문제를 푼 뒤 별표를 누르면 여기에 저장되어 언제든 다시 연습할 수 있어요.",
  reviewDoneTitle: "복습 완료!",
  reviewDoneBody: (cleared, remaining) =>
    `이번 회차에 ${cleared + remaining}문제를 복습했어요 — ${cleared}개 정답, ${remaining}개는 더 연습이 필요해요. 맞힌 문제는 오답 모음에서 빠졌고, 틀린 문제는 남아서 다시 연습해요.`,
  reviewDoneAgain: "한 회차 더 복습",
  reviewDoneExit: "홈으로 돌아가기",
  dailyDoneTitle: "오늘의 연습 완료!",
  dailyDoneBody: (cleared, remaining) =>
    `오늘 ${cleared + remaining}문제를 풀었어요 — ${cleared}개 정답, ${remaining}개 오답. 틀린 것은 약점 복습으로 들어가 다음에 다시 나와요.`,
  dailyDoneAgain: "한 세트 더",
  dailyDoneExit: "홈으로 돌아가기",
  sessionDoneTitle: "세트 완료!",
  sessionDoneBody: (cleared, remaining) =>
    `이번 세트에서 ${cleared + remaining}문제를 풀었어요 — ${cleared}개 정답, ${remaining}개 오답. 틀린 것은 약점 복습으로 들어가 다음에 다시 나와요.`,
  sessionDoneAgain: "한 세트 더",
  sessionDoneExit: "홈으로 돌아가기",
  donePerfectBadge: "만점",
  speakAriaLabel: "일본어 소리 내어 읽기",
  authSignIn: "Google로 로그인",
  authSignOut: "로그아웃",
  authSignedInAs: (name) => `${name}`,
  authSignInHint: "로그인 없이도 사용할 수 있고 기록은 이 기기에 저장돼요. 여러 기기에서 동기화하려면 로그인하세요.",
  authSyncingHint: "동기화 중…",
  authSyncedHint: "오답과 진행 상황이 기기 간에 동기화됐어요.",
  authSyncErrorHint: "동기화에 실패했어요 — 로컬 기록은 안전하며, 다음 로그인 때 다시 시도합니다.",
  authErrors: {
    sessionFetchFailed: "로그인 상태를 확인할 수 없습니다",
    authUnavailable: "로그인 서비스를 사용할 수 없습니다",
    signOutFailed: "로그아웃에 실패했습니다"
  },
  deleteHistoryLabel: "연습 기록 삭제",
  deleteHistoryTitle: "연습 기록 삭제",
  deleteHistoryDescription:
    "이 계정에 동기화된 연습 답안 기록을 삭제하고 이 기기의 연습 기록과 약점·복습 진행 상황을 지웁니다. 계정, 북마크, 언어 및 표시 설정은 삭제되지 않습니다. 이 작업은 되돌릴 수 없습니다.",
  deleteHistoryConfirm: "삭제",
  deleteHistoryConfirming: "삭제 중…",
  deleteHistoryCancel: "취소",
  deleteHistoryClose: "대화상자 닫기",
  deleteHistoryCheckbox: "이 작업은 되돌릴 수 없다는 것을 이해합니다",
  deleteHistorySuccess: "연습 기록이 삭제되었습니다",
  deleteHistoryError: "삭제에 실패했습니다. 다시 시도해 주세요.",
  conversationTitle: "일상 회화 연습실",
  conversationIntro:
    "짧은 일상 상황에서 대화를 이어 가는 연습을 합니다. 길이를 고르고 상황에 맞는 대답을 선택한 뒤, 사람이 정리한 피드백을 봅니다.",
  conversationSeasonalTitle: "Timely seasonal topics",
  conversationSeasonalEmpty: "No seasonal topic is ready to introduce right now. Choose an everyday scene instead.",
  conversationSeasonalPhases: { now: "Now", comingSoon: "Coming soon", recent: "Recent" },
  conversationStart: "이 상황 시작",
  conversationContinue: "계속",
  conversationRetry: "다른 표현으로 다시",
  conversationReset: "다시 하기",
  conversationChangeScenario: "상황 바꾸기",
  conversationPartnerLabel: "상대방 말",
  conversationLearnerRole: "내 역할",
  conversationPartnerRole: "상대방의 역할",
  conversationRelationship: "관계와 말투",
  conversationRoles: {
    "classmate": "classmate",
    "classmate planning lunch with a visiting friend": "classmate planning lunch with a visiting friend",
    "classmate sharing a recent hobby": "classmate sharing a recent hobby",
    "classmate who knows local lunch spots": "classmate who knows local lunch spots",
    "classmate who recently started knitting": "classmate who recently started knitting",
    "community class participant": "community class participant",
    "community event participant": "community event participant",
    "coworker": "동료",
    "coworker who commutes by train": "coworker who commutes by train",
    "coworker who sometimes commutes by bicycle": "coworker who sometimes commutes by bicycle",
    "coworker who uses the Aoba Line": "coworker who uses the Aoba Line",
    "junior coworker": "junior coworker",
    "local resident": "local resident",
    "member who is joining an ongoing hobby conversation": "member who is joining an ongoing hobby conversation",
    "new acquaintance who enjoys hiking but has limited energy for trips": "new acquaintance who enjoys hiking but has limited energy for trips",
    "new acquaintance who prefers nearby walks and photography": "new acquaintance who prefers nearby walks and photography",
    "new resident deciding how to commute": "new resident deciding how to commute",
    "participant": "참가자",
    "photo club member sharing a hobby": "photo club member sharing a hobby",
    "senior coworker": "senior coworker",
    "student and classmate": "student and classmate",
    "student and seminar classmate": "student and seminar classmate",
    "student with a part-time job": "student with a part-time job",
    "student working on reports": "student working on reports"
  },
  conversationChooseResponse: "대답 고르기",
  conversationFeedbackTitle: "피드백",
  conversationCompleteTitle: "완료",
  conversationSummaryTitle: "이번 연습",
  conversationPracticedLabel: "연습한 대답 동작",
  conversationCuratedNote:
    "피드백은 사람이 정리한 것으로 네트워크 없이 바로 표시됩니다. AI 채점과 음성 재생은 없습니다.",
  conversationLengths: { short: "짧음", medium: "보통", long: "김" },
  conversationDimensions: {
    understandable: "알아들음",
    correct: "정확함",
    natural: "자연스러움",
    continuation: "대화 지속",
    register_context_fit: "상황에 맞음"
  },
  conversationStatus: { met: "해냄", needs_work: "보완 필요" },
  conversationComposition: { answer: "답하기", add: "덧붙이기", ask: "되묻기" },
  homeCardConversationTitle: "일상 회화",
  homeCardConversationSub: "JLPT 문제와 별개의 짧은 일상 일본어 회화 연습.",
  homeCardConversationMeta: "짧음 / 보통 / 김",
  partOfSpeech: {
    verb: "동사",
    i_adjective: "い형용사",
    na_adjective: "な형용사",
    noun: "명사",
    adverb: "부사",
    expression: "인사 표현",
    mixed: "혼합"
  },
  verbGroups: {
    godan: "1그룹",
    ichidan: "2그룹",
    irregular: "3그룹",
    all: "전체"
  },
  focusOptions: {
    single: "단일 활용형",
    coreConjugation: "핵심 동사 활용",
    teTa: "て/た 비교",
    negative: "부정형 정리",
    plain: "보통형 정리",
    adverbial: "く/に 수식",
    obligationPast: "과거 의무"
  },
  targetForms: {
    dictionary: "사전형",
    masu: "ます형",
    nai: "ない형",
    negativeTe: "부정 て형・ないで",
    negativeContinuative: "부정 연용형・なくて",
    adverbial: "수식형・く/に",
    obligationPast: "과거 의무・なければならなかった",
    te: "て형",
    ta: "た형",
    potential: "가능형",
    volitional: "의지형",
    conditional: "가정형・ば형",
    causative: "사역형",
    passive: "수동형",
    desiderative: "희망・たい형",
    reading: "읽기",
    meaning: "뜻",
    plainPresentAffirmative: "보통형・비과거 긍정",
    plainPresentNegative: "보통형・비과거 부정",
    plainPastAffirmative: "보통형・과거 긍정",
    plainPastNegative: "보통형・과거 부정"
  },
  lessonCardFocus: ["て형 / た형 음변화", "ないで / なくて / なかった", "형용사 & 명사 활용형"]
};
