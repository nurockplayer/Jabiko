...
const NAKUTE_MO_II_ITEMS: SentencePatternItem[] = [
  ...
  },
  {
    ...
    patternId: "nakute-mo-ii",
    promptText: "このパーティーはカジュアルだから、スーツを ___ 。",
    promptContextZh: "這個派對是輕鬆場合，不必穿西裝。",
    ...
    explanation:
      "「カジュアル」表「輕鬆」，與西裝相反 → 「不必穿」用「なくてもいい」。「なければならない」是「必須穿」（與前句衝突）；「てはいけない」是「不可穿」（過度禁止）；「てもいい」是「可以穿」（語感太弱）。"
  }
];
...
const TE_MORAU_ITEMS: SentencePatternItem[] = [
  ...
  },
  {
    ...
    explanation:
      "別人（朋友）為我做 → 「てくれる」。「てあげる」是我為別人做；「てもらう」需要「に」標記做事者；「差し上げる」是「あげる」的尊敬，用於我對上位。"
  },
  {
  ...
  },
  {
    ...
    explanation:
      "「我」對年長者做善行 → 「てあげる」。注意：對長輩用「差し上げる」雖更禮貌，但對不認識的長輩、且強調善意時，「てあげる」較自然；如果是熟識長輩可改「て差し上げる」。"
  },
  {
...
const TO_OMOU_ITEMS: SentencePatternItem[] = [
  {
    ...
    options: ["と", "って", "は", "って思う"],
    explanation:
      "間接引用內容後接「と言う」。「って」是「と」的口語形（也可用）；「は」是話題助詞，不能標記引用；「って思う」是另一個句型，不能接在 言う 前。"
  },
  {
  ...
  },
  {
    ...
    patternId: "to-omou",
    promptText: "彼女は「もうすぐ ___ 」と言いました。",
    promptContextZh: "她說：「就快到了。」（直接引用）",
    expectedAnswer: "着きます",
    options: ["着きます", "着く", "着くは", "着いた"],
    explanation:
      "引號內是直接引用，可以保留原話 → 「着きます」(ます形可用)。「着く」普通形也可用，但直接引用通常保留說話者原樣語感。「着くは」不存在；「着いた」是過去式（與「もうすぐ」未來感衝突）。"
  },
  {
  ...
  },
  {
    ...
    expectedAnswer: "って",
    options: ["って", "と", "は", "って言って"],
    explanation:
      "口語引用「って」很常見，配「言ってた」(言っていた的口語簡縮) 自然。「と」也可以但較書面；「は」是話題助詞；「って言って」會變成重複（言って＋言ってた）。"
  },
  {
  ...
  },
  {
    ...
    patternId: "to-omou",
    promptText: "彼は ___ と聞いた。",
    promptContextZh: "我聽說他是學生。",
    expectedAnswer: "学生だ",
    options: ["学生だ", "学生", "学生で", "学生です"],
    explanation:
      "名詞做普通形句尾需加「だ」→ 「学生だ」。「学生」少了だ（×和と聞く一起用）；「学生で」是 て形（不能直接接と）；「学生です」是丁寧形（與普通形相比較少見，但口語可）。"
  }
];
