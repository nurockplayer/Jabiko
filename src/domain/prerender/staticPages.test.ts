...
describe("buildStaticPages", () => {
  ...
  it("prerenders the partnership list framing and the author's own contact address", () => {
    const partners = byPath.get("/stay-d");
    ...
    expect(partners!.bodyHtml).toContain("合作洽詢請直接寄到這個信箱。");
    // Ownership is carried by the section, not by a disclaimer sentence: the
    // address sits under its own 想和 Jabiko 合作？heading, never in a card.
    expect(partners!.bodyHtml).toContain("想和 Jabiko 合作？");
