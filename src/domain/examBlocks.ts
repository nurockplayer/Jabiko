...
,
  ...
  examQuestion({
    ...
    promptLabel: "文法形式選擇",
    instructionZh: "句中填空：依文脈選最自然的文法。",
    promptText: "けさは 九時 ___ はたらきました。",
    promptContextZh: "今天早上工作到九點。",
    hintZh: "標示「工作」這個動作一直持續到的那個終點時刻。",
    expectedAnswer: "まで",
    options: ["まで", "から", "より", "だけ"],
    explanation: "「Ｎまで」用「まで」表示動作持續到的終點，「九時まではたらきました」＝工作到九點為止。「から」是起點「從…」、與『到…為止』方向相反；「より」是「比…」、句中無比較；「だけ」是「只…」、非範圍終點。表持續終點唯「まで」成立。"
  }),
  examQuestion({
  ...
  examQuestion({
    ...
    promptLabel: "文法形式選擇",
    instructionZh: "句中填空：依文脈選最自然的文法。",
    promptText: "旅行の 前に、ガイドブックを 読んで 行く 店を 決めて ___ 。",
    promptContextZh: "旅行前先把要去的店決定好。",
    hintZh: "為了之後的旅行，現在先把事情安排妥當。",
    expectedAnswer: "おきます",
    options: ["おきます", "しまいます", "みます", "あります"],
    explanation: "「Vておく」表示為了將來做好準備，本句「旅行前先決定好要去的店」正是事前準備，故「決めておきます」。「しまいます」（てしまう）表完成或遺憾，「決めてしまいます」語感是『不小心決定了／乾脆決定了』，與『為旅行有計畫地準備』不合；「みます」（てみる）是「試試看決定」，但句意是把事情敲定而非嘗試；「あります」（てある）要接他動詞表狀態留存（決めてある也表已決定的狀態），但與「旅行の前に」這個『現在著手準備』的時間點搭配，表動作目的的「ておく」更貼切。為將來準備唯「おきます」成立。"
  }),
  examQuestion({
