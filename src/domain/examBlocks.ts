...
,
  ...
  examQuestion({
    ...
    expectedAnswer: "ばかりに",
    options: ["ばかりに", "だけに", "おかげで", "ものだから"],
    explanation: "「Vたばかりに」強調「只因一個看似不大的失誤，就釀成令人懊悔的壞結果」，本句「多一句話→生意鬧僵」正是此惋惜語感。「だけに」是「正因…程度更…」（需有程度／情緒加倍的對比，此處無）；「おかげで」通常引出正面結果；「ものだから」是辯解原因，無「小因大果」的後悔。唯「ばかりに」成立。"
  }),
  examQuestion({
    ...
    expectedAnswer: "だけに",
    options: ["だけに", "ばかりに", "だけあって", "とあって"],
    explanation: "「〜だけに」是「正因為前項程度深，所以後項結果（此處的失落）更加倍」的強對比因果。「ばかりに」要求後項壞結果直接由前因造成（中止並非籌備所致，不合）；「だけあって」結論須是「不愧是…」的正面成果（「落胆」是負面情緒，不合）；「とあって」是「正因發生某既成事件」的新聞式因果。唯「だけに」表情緒加倍。"
  }),
  examQuestion({
    ...
    promptLabel: "文法形式選擇",
    instructionZh: "句中填空：依文脈選最自然的文法。",
    promptText: "長年支えてくださったお客様のご要望 ___ 、休日を返上してでも対応するつもりだ。",
    promptContextZh: "若是多年來一直支持我們的客戶提出的要求，就算犧牲休假我也打算去處理。",
    hintZh: "面對老客戶的要求，自己願意付出到什麼程度。",
    expectedAnswer: "とあれば",
    options: ["とあれば", "とあって", "にあって", "ときたら"],
    explanation: "「Nとあれば」表示「若是為了 N 這種情況，就不惜…」，後句常配「就算犧牲…也要做」。「とあって」是「正因發生 N（既成事實），所以…」（事件報導因果，本句是假設並非既成事實，不合）；「にあって」是「處於 N 的情況下」（場面背景）；「ときたら」是「說到 N（多帶不滿）」。本句的「為了客戶不惜犧牲休假」唯「とあれば」吻合。"
  }),
  examQuestion({
  ...
  examQuestion({
    ...
    expectedAnswer: "がち",
    options: ["がち", "ぎみ", "かけ", "つつ"],
    explanation: "「Vます形 / N + がち」表示「往往、容易（出現不太理想的傾向）」，本句「越熟練越常省略確認」正是這種習慣性傾向。「ぎみ」是「略帶…的感覺」（程度輕，如「疲れぎみ」），不表頻繁傾向；「かけ」是「做到一半」（如「やりかけ」）；「つつ」是「一邊…一邊…／儘管…」。表負面習慣傾向唯「がち」吻合。"
  }),
  examQuestion({
    ...
    expectedAnswer: "にもまして",
    options: ["にもまして", "もさることながら", "を限りに", "に至っては"],
    explanation: "「N にもまして」表示「比起本就（很高的）N，還更…」，本句「上一期已暢旺，這一期更勝」正是此義。「もさることながら」是「A 固然…，B 更…」，須並列兩個主題且 B 為另一事項（此處只比較前後期同一指標，不合）；「を限りに」是「以…為界（從此不再）」；「に至っては」是「至於…更是（極端舉例）」。表「比已極端的前項更甚」唯「にもまして」吻合。"
  }),
  examQuestion({
  ...
  examQuestion({
    ...
    expectedAnswer: "の至り",
    options: ["の至り", "の極み", "を禁じ得ない", "極まりない"],
    explanation: "「N の至り」用於正式致辭，接「光栄／感激／恐縮」等表「…之至」的客套極致，本句「光栄の至り」即「無上榮幸」。「の極み」雖也表極致，但多接「贅沢／疲労／混乱」等（偏負面或感官狀態），與「光栄」搭配不自然；「を禁じ得ない」須接情緒名詞（怒り／涙）；「極まりない」接形容詞語幹（如「失礼極まりない」），不接「光栄」這個名詞。致辭客套唯「の至り」吻合。"
  }),
  examQuestion({
  ...
  examQuestion({
    ...
    promptLabel: "文法形式選擇",
    instructionZh: "句中填空：依文脈選最自然的文法。",
    promptText: "ミスを減らすため、退社前には必ず翌日の予定を見直す ___ している。",
    promptContextZh: "為了減少失誤，我平時都會努力做到下班前一定重看隔天的行程。",
    hintZh: "為減少失誤而在日常中持續維持的一個習慣動作。",
    expectedAnswer: "ようにして",
    options: ["ようにして", "ことにして", "ようになって", "ことになって"],
    explanation: "「Vる + ようにしている」表示「（靠自身努力）養成並持續維持某習慣」，本句「平時都盡量做到下班前重看行程」正是此義。「ことにしている」雖也表習慣，但語感是「（基於一次決定而）規定自己這麼做」，本句強調的是「努力做到」的持續經營；「ようになっている／ことになっている」是「（自然）變成…／（規定）成為…」（非自身刻意維持的習慣）。表持續努力的習慣唯「ようにして」最自然。"
  })
];
